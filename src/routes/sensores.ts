...
export async function sensoresRoutes(app: FastifyInstance) {
  ...
  app.put('/:id', { preHandler: auth }, async (request, reply) => {
    ...
    const schema = z.object({ codigo: z.string().optional(), tipo: z.string().optional() })
    const result = schema.safeParse(request.body)
  ...
  app.post('/data', async (request, reply) => {
    ...
    if (!sensor) return reply.status(401).send({ message: 'Token inválido' })
    ...
    for (const alerta of alertas) {
      ...
      if (!existe) await prisma.alerta.create({ data: { ...alerta, fazendaId: sensor.fazendaId } })
    }
